...
namespace Ria
{
    ...
    /// </summary>
    [System.Serializable]
    public class TestEnemyManager : TestChildManager
    {
        ...
        {
            var enemy = GameObject.Instantiate(_obj);
            enemy.SetActive(false);
            enemy.transform.parent = this.trans;
        ...
        public void Circle(int _appearNum, Vector2 _appearPos, Vector2 _posInterval)
        {
            ...
            for (int i = 0; i < this.cEnemys.Length; i++)
            {
                ...
                if (!this.cEnemys[i].activeSelf)
                {
                    ...
                    this.cEnemys[i].GetComponent<CircleEnemy>().CenterPos = this.trans.position;
