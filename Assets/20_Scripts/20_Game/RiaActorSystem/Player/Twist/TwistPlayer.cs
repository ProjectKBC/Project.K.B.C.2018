...
namespace Game.Player
{
	...
	public sealed class TwistPlayer : RiaPlayer
	{
		// 通常ショット
		...
		// CharacterScriptの上書き
		private new TwistPlayerScript Script;
		...
		public TwistPlayer(GameObject _go, RiaCharacterScript _script, PlayerNumber _playerNumber) : base(_go, _script, _playerNumber)
		{
			// CharacterScriptの上書き
			this.Script = _script as TwistPlayerScript;
		...
		private void NormalShot()
		{
			...
			if (RiaInput.Instance.GetPush(RiaInput.KeyType.NormalShot, this.PlayerNumber))
			{
				...
				if (script.shotInterval <= shotElapsedTime)
				{
					...
					this.BulletManger.CreateTwistBullet(
